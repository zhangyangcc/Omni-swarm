#!/usr/bin/env python3read_pose_swarm_fused
import rosbag
import numpy as np
from math import *
...
def read_pose_swarm_fused(bag, topic):
    pos = {}
    ypr = {}
    ts = {}
    quat = {}
    ...
    for topic, msg, t in bag.read_messages(topics=[topic]):
            for i in range(len(msg.ids)):
                ...
                _ts.append(msg.header.stamp.to_sec())
...
def read_pose_swarm_frame(bag, topic):
    pos = {}
    ypr = {}
    ts = {}
    quat = {}
    ...
    for topic, msg, t in bag.read_messages(topics=[topic]):
            ...
            for node in msg.node_frames:
                ...
                if node.vo_available:
                    ...
                    _ts.append(node.header.stamp.to_sec())
    ...
    with open(filename, 'w') as writer:
        for i in range(len(poses["t"])):
            if i % skip == 0:
                ...
                t = poses["pos"][i]
                q = poses["quat"][i]
                writer.write("{:.9f} {} {} {} {} {} {} {}\n".format(ts,t[0],t[1],t[2],q[1],q[2],q[3],q[0]))
...
def bag2dataset(bagname, nodes = [1, 4]):
    ...
    poses_fused = read_pose_swarm_fused(bag, "/swarm_drones/swarm_drone_fused")
    ...
    for i in nodes:
        ...
        # the value of parameter skip is 10 
        output_pose_to_csv("/home/zy/ros_ws/omni_swarm_ws/output/sim_test_output/evaluate_dataset/stamped_traj_vo_{}.txt".format(i), poses_vo[i])
...
#                 poses_gt[i], t0 = read_pose(bag, "/SwarmNode{}/pose".format(i), t0, te, P_vicon_in_imu[i])
#             else:
#                 poses_gt[i], t0 = read_pose(bag, "/SwarmNode{}/pose".format(i), t0, te)
#             sum_len += poses_length(poses_gt[i])

#         if is_pc:
#             poses_path[i] = read_path(bag, "/swarm_drones/est_drone_{}_path_pc".format(i), t0, te)
#         else:    
#             poses_path[i] = read_path(bag, "/swarm_drones/est_drone_{}_path".format(i), t0, te)
